...
def make_plot():
    ...
    pot_lat=lat_pattern(ds005.potential.values)
    pot_pos_lat=lat_pattern(ds005.potential.where(ds005.potential>0).values)
    pot_neg_lat=lat_pattern(ds005.potential.where(ds005.potential<0).values)
...
#    ax2.fill_betweenx(pot_lat[0,:], pot_lat[4,:], pot_lat[5,:], facecolor='#D2D2D2',edgecolor='grey', alpha=0.8, label='Total')
    ...
    pos4 = [0.55, 0.125, 0.3, 0.5] # [left, bottom, width, height]
    ...
    ax3.set_title('MSG cloud impact ')
    ax4.set_title('Hours of MSG maximum impact')
    
    # plt.savefig('../figure/figure1.pdf', bbox_inches='tight')
    plt.savefig('../figure/figure1_neg_0512.png', dpi=300, bbox_inches='tight')
